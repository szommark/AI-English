...
import { localizeCategory, localizeFeature, useLanguage } from '../lib/i18n'
...
export default function ComingSoonPage() {
  ...
  const title = feature ? localizeFeature(lang, feature).title : localizeCategory(lang, category!)
  ...
  return (
    <div className="flex items-center justify-center py-8">
      <div className="max-w-md w-full rounded-2xl border border-slate-200 bg-white shadow-sm p-8 text-center space-y-4">
        ...
        <h1 className="text-2xl font-bold text-slate-800">{t('comingSoonTitle')}</h1>
        <p className="text-sm text-slate-500">{t('comingSoonBody')}</p>
      </div>
    </div>
  )
}
