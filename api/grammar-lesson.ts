...
import { callGroqGrammarLesson, parseGrammarLessonJson } from './_lib/groq.js'
import { getGrammarItem } from '../src/data/grammarCurriculum.js'
import type { GrammarLesson } from '../src/lib/types.js'

const GENERATION_ATTEMPTS = 2

async function readCachedLesson(cefrLevel: string, grammarItemId: string): Promise<GrammarLesson | null> {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    const lesson = await readCachedLesson(curriculumEntry.level, curriculumEntry.item.id)
  ...
  const cached = await readCachedLesson(curriculumEntry.level, curriculumEntry.item.id)
  if (cached) {
  ...
  for (let attempt = 0; attempt < GENERATION_ATTEMPTS; attempt++) {
    try {
      const result = await callGroqGrammarLesson(curriculumEntry.item, curriculumEntry.level)
